...
**Two drag modes (preference `dragMode`).** This section describes the **reflow (Beta)** mode (`dragMode: 'reflow'`). The default **classic** mode (`dragMode: 'classic'`, = the 26.5.26 experience) does NOT fold/reflow: the dragged row stays in place (dimmed `.dragging`), and the drop slot is shown by a blue insertion line on the target row's edge — `.drag-over-top` (before) / `.drag-over-bottom` (after) with an accent dot (`::before`/`::after`), restored in `contentStyleText`. `.drag-into` (folder header highlight) is shared by both modes; the reflow `.sp-drag-guide` bar, `.sp-drag-folded` fold, `.sp-drop-shift`/`.sp-drop-shift-static` reflow, and post-drop opacity/FLIP finish are reflow-only (gated on `getDragMode()`). Classic also never positions a loose source at root — `computeDropIntent` demotes such a drop to the bottom Ungrouped bin (v4 behavior). The toggle lives in Settings → Appearance ("Enable reflow drag (Beta)", default off); the What's-New modal offers a one-tap enable. Switching to Classic and every subsequent notebook load enforce the same placement invariant: after preferences and any deferred state restore are verified, positioned root sources are checkpointed and swept into the Ungrouped bin (`sweepPositionedRootSourcesToBin`). An unavailable or failed preference load never treats the in-memory Classic default as verified and therefore never sweeps. Preference LOAD/SAVE responses use monotonic request ordering: an explicitly successful newer SAVE supersedes an older pending LOAD, a partial successful response does not mark the full preference schema as verified, and a failed SAVE does not invalidate an earlier LOAD. The migration save is immediate, critical, excluded from undo history, and retains recovery data if persistence fails. A Classic mode change reports success only after that invariant succeeds and the persisted return plus current runtime mode both equal the requested mode; `not_classic` is successful only for a requested Reflow mode. Verification, stale-instance, mode-mismatch, checkpoint, sweep, or save failures reject the setting change and attempt to restore the previous mode, while the toggle always reflects the actual runtime mode. Panel reattachment runs the invariant before deciding whether to persist: a successful migration reuses its single critical save, and only reflow or an already-valid Classic tree receives the ordinary reattach save. Immediately before that extra save, the reattach continuation revalidates the bound project and manager instance so an old panel cannot persist into a newly selected notebook.

Classes: `.dragging` (state marker), `.sp-drag-folded`, `.sp-drag-unfolding`, `.sp-drop-shift` (viewport-near animated shift), `.sp-drop-shift-static` (offscreen non-animated shift), `.sp-drag-ghost`, `.sp-drag-ghost-single`, `.sp-drag-ghost-stack`, `.sp-drag-ghost-layer`, `.sp-drag-ghost-badge`, `.drag-into`, `.drag-invalid`, `.drag-over-top` / `.drag-over-bottom` (classic blue insertion line), `.sp-pseudo-hover` (post-drop), `.sp-drag-active` (during-drag + post-drop, on `#sources-list`), `.sp-drag-guide` (folder left-guide extension while a child is folded), `.sp-ungroup-dropzone` (transient "move to ungrouped" hint mounted at the list bottom when the ungrouped bin is empty)
...
- Native `DataTransfer.dropEffect` is written only inside the original `handleDragOver` event. A clean cached snapshot lets `resolveSynchronousDropEffect` recompute the current pointer without any DOM read and immediately switch between `none` and `move`; missing, dirty, stale-session, or unusable geometry uses conservative `move`, while an unsupported internal drag context uses `none`. The queued drag frame updates only internal intent and visual feedback — it must never retain or asynchronously rewrite `DataTransfer`. A dirty drop synchronously flushes fresh geometry before mutation and still fails closed when that read cannot be verified.
- Successful edge auto-scroll reports its exact before/after position through the controller, invalidates geometry, and schedules the existing coalesced drag frame using the stationary pointer. The controller callback and native `scroll` listener share one baseline so the same delta is never patched twice; reaching a boundary or ending the drag stops both the scroll loop and pending refresh. If drop arrives before that refresh frame, the same synchronous flush consumes the new scroll position first.
- Siblings at and after the target insertion index translate down by the session's measured `totalDraggedHeight`. Rows intersecting the list viewport plus one physical row-height overscan use `.sp-drop-shift`; shifted rows farther away use `.sp-drop-shift-static`, which preserves the same geometry without scheduling hundreds of offscreen transitions. A static class may remain briefly after its transform is cleared so the base row transition cannot re-enable in the same style flush; `clearReflow` and the next-drag preflight remove every residual class. The visible gap therefore matches mixed source/group rows, collapsed margins, and content-box/border-box items instead of assuming `N × itemHeight`; there is no separate insertion bar.
- The folder left-guide bar tracks the **drop target**, not the origin. Each frame `_processDragOver` marks the folder the pointer is currently inside (`intent.hostGroupContainerEl` — `into-group` or a slot within an expanded folder) with `.sp-drag-guide` + `--sp-slot-comp` (= `totalDraggedHeight`), clearing the previous frame's marker first. CSS draws that folder's guide as an absolute `::before` of `height: calc(100% + var(--sp-slot-comp))`, colored blue — so it spans the folder's content **plus one insertion slot**, previewing where the source lands (including the empty slot when dropping at the very end), **without entering layout** (cannot disturb reflow / cross-host shifts). Every other folder keeps its plain grey `border-left`, which naturally ends at its last item. Because Chrome freezes native `:hover` on the origin folder at dragstart, `handleDragStart` adds `.sp-drag-active` to `#sources-list` for the whole drag so `#sources-list.sp-drag-active` can suppress that frozen `:hover` blue (the bar is owned exclusively by `.sp-drag-guide`). The guide is cleared per-frame + on dragend (`clearDragFeedback`), with preflight as a backstop; a valid replacement drag reuses an existing `.sp-drag-active` instead of restyling the whole list off and on, while a guarded start and the post-drop trusted-pointer cleanup still remove it.
- The drag ghost is a 1:1 clone of the actual source-item row (icon, title, tags) with a fixed allowlist of layout, box, flex/grid, typography, color, overflow, and object-fit computed styles copied inline so it renders consistently outside the Shadow DOM without serializing every computed property. Single-source drag wraps the clone with `.sp-drag-ghost-single` (transform scale 0.95 + drop-shadow). Multi-source drag stacks the first three clones via `.sp-drag-ghost-stack`: the top layer (`:nth-child(1)`) is only scaled (`scale(0.95)`, no offset/rotate), while layers 2 and 3 are offset + rotated + scaled and dimmed (opacity `0.85` / `0.7`). A circular `.sp-drag-ghost-badge` at top-right shows the total selection count N.
...
- On dragend cancel (esc / drop outside the list), `unfoldDraggedItems({ animated: true })` swaps `.sp-drag-folded` → `.sp-drag-unfolding` and writes the measured `unfoldHeight` + opacity 1 so content-box and border-box rows grow back to the same outer size on the shared base-motion beat (`var(--sp-motion-base)` = 180ms). Timeout cleanup restores the original inline height/opacity values, priorities, and pre-existing fold/unfold class membership. `animated: false` suppresses motion through the synchronous restore. Both paths pair with `clearReflow` so shifted siblings return without an overshoot.
- `handleDragStart` preflights with one combined descendant query, stripping lingering drag-feedback classes and resetting their backing inline styles — `.sp-drag-folded` (+ height/opacity), `.sp-drop-shift` / `.sp-drop-shift-static` (+ transform), `.sp-drag-unfolding`, `.sp-pseudo-hover`, `.sp-hover-expand-pending`, `.sp-drag-cancelled` (plus legacy-defensive removes of `.sp-drop-flying` / `.sp-drop-landing`, which are no longer applied). A valid replacement drag preserves the host-level `.sp-drag-active` needed by the new session; an editable/checkbox-guarded start clears it before returning. This prevents an interrupted prior drag from leaking a stuck-folded row, offset sibling, or frozen outline without performing five whole-tree sweeps.
...
- **Fold layout is synchronous; opacity is animated**: `.sp-drag-folded` applies terminal zero box geometry in the deferred fold frame and transitions only opacity on `var(--sp-motion-base) var(--sp-ease-emphasized)`. Do not animate height/padding/margin/border during the active drag.
- **Unfold (`.sp-drag-unfolding` on cancel)** and **viewport-near reflow shift (`.sp-drop-shift`)** transition on `var(--sp-motion-base) var(--sp-ease-emphasized)` (the shared base-motion beat), so the dragged item's grow-back and visible siblings' slide-back stay coordinated. `.sp-drop-shift-static` has `transition: none !important`; the post-drop landed-row fade-in is opacity-only on the same base beat.
- Do not apply blanket `will-change: transform` to every shifted row. Large multi-select drags can shift hundreds of offscreen rows; promoting all of them increases layer and style-flush cost. Animation scope is the viewport plus one row of overscan.
...
- `@media (prefers-reduced-motion: reduce)` disables drag transitions (`.sp-drop-shift`, `.sp-drop-shift-static`, `.sp-drag-unfolding`, `.group-container.drag-into > .group-header`) and shows `.sp-ungroup-dropzone` instantly (`animation: none` — it still appears, just without the entry beat).

Implementation notes:

- Reflow writes live in `src/content/content-drag-reflow.js` (`prepareDragSession`, `foldDraggedItems`, typed `applyReflow`, `clearReflow`, `unfoldDraggedItems`). `content-tree-interactions.js` owns `readDragGeometry`, `computeDropIntentRaw`, `resolveSynchronousDropEffect`, `planDragFrame`, `applyDragFramePlan`, `invalidateDragGeometry`, and `flushDragFrameNow`, and calls the reflow writer from dragstart / dragover / drop / dragend.
- A clean frame reuses the cached geometry and validates the root rect once. Exact root scroll deltas translate every cached rect; a nested `.group-children` scroll delta translates only its descendants. Root size changes, nested-scroller size changes, render replacement, hover layout changes, mixed scroll/non-scroll invalidation, identity mismatch, or incomplete transform patch rebuild the snapshot fail closed. A drag-scoped passive scroll listener and lazy `ResizeObserver` are installed from the first snapshot and removed on teardown.
- `prepareDragSession` caches row references only while `root.contains(row)` and the exact `data-source-key` still match; replaced rows are re-resolved. Immutable item metrics are reused only when every measured geometry, box-model, inline priority, and class scalar is unchanged. These caches reduce repeated 50-row prepare allocations without allowing stale DOM or stale layout values into a new session.
- Ghost cloning lives in `src/content/content-drag-multi.js` (`cloneSourceItem` / `inlineStylesRecursive`). The recursive helper copies only `GHOST_STYLE_PROPERTIES`; `createMultiDragGhost({ count, sourceClones, root })` builds the single-vs-stack wrapper + badge from pre-cloned Elements.
- Insertion position is computed by `computeDropIntentRaw` in `content-tree-interactions.js` as a pure pointer-Y → list → slot mapping over the frame's transform-neutral `layoutRect` values. `readDragGeometry` derives each layout rect by subtracting both own and inherited translateY from its visual rect, so nested headers/children remain in the same coordinate frame as their shifted ancestor containers. The deepest matching group wins via parent-map depth, then the direct children's visual midpoints choose the preview slot. Never derive intent from `e.target.closest()` or perform a second rect scan during dragover; the rendered layout includes transform shifts and every geometry read must finish before the first write.
- Filtered slot mapping uses the last visible row as the insertion anchor rather than treating it as the end of the hidden underlying container. The pre-decision characterization found root source/group paths already returned index `3` for the example below, while group children returned `4`; an empty visible list over nonempty state guessed index `0`. `resolveVisibleAnchorInsertIndex` now applies one identity-safe rule to root, group children, single-source, multi-source, and group reorder paths, and returns no target when a nonempty container has no visible anchor. Same-container removal/index correction remains a later mutation-stage concern and is not part of the anchor resolver.
  ...
  ```
- `.dragging`, `.sp-drag-folded`, `.sp-drag-unfolding`, `.sp-drop-shift`, `.sp-drop-shift-static`, `.drag-into`, `.drag-invalid`, `.drag-over-top`, `.drag-over-bottom`, `.sp-pseudo-hover`, `.sp-drag-active`, `.sp-drag-guide` styles all live in `contentStyleText` (Shadow DOM scope).
...
- Do not replace the measured footprint with `marginTop + marginBottom`, `N × offsetHeight`, or `next.top - first.top`; collapsed margins, mixed source/group rows, nested hosts, and content-box sizing require the real before/folded displacement.
- Do not add a geometry read after `applyDragFramePlan` begins writing. Patch pure transform deltas in memory; invalidate and rebuild on the next frame when a mutation cannot be represented exactly.
- Do not move native `dropEffect` writes into `requestAnimationFrame`, auto-scroll callbacks, frame application, drop, or dragend. The browser accepts reliable feedback only in the originating `dragover` lifecycle, and retaining `DataTransfer` across that boundary can replay a stale effect.
- Do not remove `.sp-drop-shift-static` in the same style batch that clears an offscreen transform; doing so re-enables the base row transition and can animate hundreds of rows.
