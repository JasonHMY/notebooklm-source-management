...
- The guide bar's blue accent follows the **target** folder, not the origin. Chrome freezes native `:hover` on the folder under the cursor at dragstart, so a `:hover`-driven blue bar would stay stuck on the source folder. `handleDragStart` therefore adds `.sp-drag-active` to `#sources-list` for the **whole drag** (previously post-drop only); CSS scoped to `#sources-list.sp-drag-active` suppresses the frozen `:hover` accent and instead lights the guide of whichever folder currently has `.drag-into` (the pointer's live target, set per-frame by `_processDragOver`). `.sp-drag-active` is torn down by the same post-drop trusted-pointer cleanup / preflight as before.
