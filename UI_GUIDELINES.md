...
- The formal fold remains deferred to the next animation frame so Chrome can finish native drag-image capture: `height`/padding/margin/border collapse to `0` and `opacity` fades to `0` (`.sp-drag-folded`). Multi-source drag folds every selected row in the same frame.
- During dragover, siblings at and after the target insertion index translate down by the session's measured `totalDraggedHeight` (`.sp-drop-shift` with inline `transform: translateY(...)`). The visible gap therefore matches mixed source/group rows, collapsed margins, and content-box/border-box items instead of assuming `N × itemHeight`; there is no separate insertion bar.
