...
- **Native NotebookLM DOM overrides**: [src/content/styles.css](src/content/styles.css) is injected directly by `manifest.json` `content_scripts[0].css` (NOT a Shadow-DOM/JS template). It uses `!important` under the `.sources-plus-manager-active` host class to hide/neutralize the native source panel (kept in the render tree, not `display:none`, so Angular CDK can still measure overlays). This must live in the page, not the Shadow DOM, to override NotebookLM's own styles — keep page-level native overrides here, not in the two JS template strings above.
