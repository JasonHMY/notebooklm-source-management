...
- **Drag ghost 视觉 + edge scroll 阈值调整 (Drag Ghost Polish + Edge Scroll Threshold Tweak)**: 按用户 UX spec 调整两条独立细节。(1) 单源 ghost `.sp-drag-ghost-single > .sp-drag-ghost-layer` 改为 `transform: scale(1.02) rotate(1.5deg) + opacity: 0.7 + box-shadow: 0 8px 24px rgba(0,0,0,0.25)`，模拟「被拎起来」的物理感，并把不透明度降到 0.7 让 ghost 下方目标行依旧可读（原来 scale(0.95) + drop-shadow filter 过实）。(2) `createMultiDragGhost` 的 edge auto-scroll 默认值从 `edge 60px / max 14px·frame⁻¹` 改为 `edge 40px / max 8px·frame⁻¹`，更靠近 cursor 才触发滚动、滚动速度更平缓。`computeAutoScrollVelocity` 算法不变，单测继续传显式参数（不受默认值影响）。
- **setDragImage offset 补偿 ghost shadow 扩展 (setDragImage Offset Shadow-Pad Compensation)**: 修复用户反馈的「ghost 偏在鼠标光标右下方 20–40px」。根因：`setDragImage(image, offsetX, offsetY)` 的 offset 是**捕获位图**坐标，而浏览器捕获 ghost 时包含 `box-shadow` / `drop-shadow filter` 渲染的所有像素 —— shadow 向左 / 上扩展（box-shadow `0 8px 24px` 向左 24px / 向上 16px；filter `0 4px 12px` 向左 12px / 向上 8px），所以 source-item 内容在捕获 image 中并不在 (0, 0) 而是偏右下 (shadow_left_pad, shadow_top_pad)。补偿：根据 ghost 是否含 `.sp-drag-ghost-single` class 选 single (24, 16) 或 stack (12, 8) 的 padding 值，加到原来「click 在 source-item 内的相对位置」之上一并传给 `setDragImage`。三处对应单测（multi-source default offset / clone-and-offset / fallback）的期望值同步更新。
- **父子分组 drop intent X 坐标判定 (X-Coord Parent/Sub-Group Drop Detection)**: 实现 spec「拖到子分组的缩进范围内 → 落点指向子分组；落到父分组缩进范围内（更靠左）→ 落点指向父分组」。`computeDropIntent` 加 `clientX` 参数；选择最深 group-container 时除了 Y 包围检测，还要求 cursor X 落在 container 的 `.group-children` 左边以右（每层 nested group 通过 `margin-left: 18px + padding-left: 8px` 产生 26px 缩进）。X 在 children-area 左边的 cursor 表示用户「在父级缝隙里」，跳过该候选 → 回退到上层 container。`handleDragOver` 与 `handleDrop` 的 `computeDropIntent` 调用都传 `e.clientX`。clientX 为空（fallback path）时仍按 Y-only 选 deepest，与旧行为兼容。
- **拖动时分组计数乐观更新 (Optimistic Group-Count Update During Drag)**: 实现 spec「拖动过程中根据当前 drop target 做乐观计数，源分组显示 (原值-1)/y，目标分组显示 (原值+1)/y，拖回原位则恢复」。新增 `updateOptimisticGroupCounts({ intent, draggedKeys, rootElement })` helper：dragover 时直接修改 group header 的 `.badge` textContent；首次修改前用 `runtime.dragCountSnapshot` Map 记录原文本以备还原；预测目标变化时把上一帧不再涉及的 group 还原。`restoreOptimisticGroupCounts` 在 dragover 出列表 / dragend / cancel 时统一还原（drop 成功路径走 `render()` 重建，badge 自然带新真实计数；snapshot Map 仍被清掉为下次 drag 复用）。Group drag 不处理（spec 只描述 source 计数；group 移动会同时改多个层级，复杂度不值得）。Total `y` 保持不变以严格匹配 spec 表达式。
